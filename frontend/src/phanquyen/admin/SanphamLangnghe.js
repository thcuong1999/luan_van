...
const SanphamLangnghe = (props) => {
  ...
  const fetchDsSanpham = async () => {
    setLoading(true);
    const data = await apiSanphamLangnghe.dsSanpham();
  ...
  return (
    <>
      <Wrapper>
        <Header title="Sản phẩm làng nghề" />
        <Content>
          <BtnRight>
            <ButtonMaterial
              variant="contained"
              onClick={() => props.history.push("/admin/sanphamlangnghe/them")}
            >
              Thêm sản phẩm
            </ButtonMaterial>
          </BtnRight>
...
export default SanphamLangnghe;
