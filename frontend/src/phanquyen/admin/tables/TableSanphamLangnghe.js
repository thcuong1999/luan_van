...
import { headCellsSanphamLangnghe } from "./headCells";
...
const TableSanphamLangnghe = ({ dsSanpham = [], setRowsRemoved, setAlert }) => {
  ...
  const onClickChitiet = () =>
    history.push(`/admin/sanphamlangnghe/chitiet/${selected[0]}`);

  const onClickCapnhat = () =>
    history.push(`/admin/sanphamlangnghe/chinhsua/${selected[0]}`);

  const onClickXoa = () => handleOpen();

  const handleDeleteRow = async () => {
    const data = await apiSanphamLangnghe.xoaNhieuSanpham({
      arrOfIds: selected,
    });
    if (data.success) {
      setAlert(true);
          ...
          <TableContainer>
            ...
            >
              <EnhancedTableHead
                ...
                headCells={headCellsSanphamLangnghe}
              />
              <TableBody>
                {dsSanpham
                  .slice()
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="right">
                          <Link
                            to={`/admin/sanphamlangnghe/chitiet/${row._id}`}
                          >
                            {row.ten}
                          </Link>
                        </TableCell>
...
export default TableSanphamLangnghe;
