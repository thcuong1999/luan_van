...
import { headCellsDsBophankd } from "./headCells";
              ...
              <DropdownCustom
                ...
                data={
                  rowsSelected.length === 1
                    ? ["Chi tiết BPKD", "Cập nhật BPKD", "Xóa BPKD"]
                    : ["Xóa BPKD"]
                }
...
const TableBophankd = ({ dsBophankd, setRowsRemoved }) => {
  ...
  const toolbarSelectedDropdownVal = (val) => {
    if (val === "Chi tiết BPKD") {
      history.push(`/admin/bophankd/chitiet/${selected[0]}`);
    } else if (val === "Cập nhật BPKD") {
      history.push(`/admin/bophankd/chinhsua/${selected[0]}`);
    } else if (val === "Xóa BPKD") {
      handleOpen();
    }
  };

  const handleDeleteRow = async () => {
    const data = await apiBophankd.xoaNhieuBophankd({ arrayOfId: selected });
    if (data.success) {
      Toastify({
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = dsBophankd?.map((item) => item._id);
          ...
          <TableContainer>
            ...
            >
              <EnhancedTableHead
                ...
                rowCount={dsBophankd?.length}
                headCells={headCellsDsBophankd}
              />
              <TableBody>
                {dsBophankd
                  ?.slice()
