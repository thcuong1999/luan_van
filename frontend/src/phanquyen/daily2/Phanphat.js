...
const Phanphat = (props) => {
  ...
  const fetchDaily1Info = async () => {
    setLoading(true);
    const data1 = await apiDaily2.singleBophankdBasedUser(userInfo._id);
    const data2 = await apiPhanphat.dsPhanphatDenDaily2(data1.daily2._id);
    setDsPhanphatDen(
      data2.phanphat.map((item) => ({
        ...item,
        trangthaiQuery:
