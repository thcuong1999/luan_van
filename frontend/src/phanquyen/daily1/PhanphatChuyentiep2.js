...
const PhanphatChuyentiep2 = (props) => {
  ...
  const handleTienhanhPhanphat = async () => {
    ...
    const data = await apiPhanphat.daily1ppvattuDaily2(payload);
    // console.log(data);
    if (data.success) {
      Toastify({
      ...
      props.history.push(`/daily1/vattuphanphat/chitiet/${phanphatId}`);
    }
  };
  ...
  return (
    <>
      <Wrapper>
        <Header
          title="Quay về chi tiết phân phát"
          titleBack
          onClick={() =>
            props.history.push(`/daily1/vattuphanphat/chitiet/${phanphatId}`)
          }
        ...
        <Content>
          ...
          <Section>
            <SectionTitle>Danh sách vật tư phân phát</SectionTitle>
            <TableSection>
