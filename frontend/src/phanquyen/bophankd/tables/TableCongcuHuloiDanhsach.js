...
const TableCongcuHuloiDanhsach = ({ dsCongcuHuloi = [] }) => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = dsCongcuHuloi?.map((item) => item._id);
  ...
  const emptyRows =
    page > 0
      ? Math.max(0, (1 + page) * rowsPerPage - dsCongcuHuloi?.length)
      : 0;
...
export default TableCongcuHuloiDanhsach;
