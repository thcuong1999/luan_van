...
import { headCellsCongcuHuloi } from "./headCells";
...
const TableVattuHuloiDanhsach = ({ dsVattuHuloi = [] }) => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = dsVattuHuloi?.map((item) => item._id);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - dsVattuHuloi?.length) : 0;
          ...
          <TableContainer>
            ...
            >
              <EnhancedTableHead
                ...
                rowCount={dsVattuHuloi?.length}
                headCells={headCellsCongcuHuloi}
              />
              <TableBody>
                {dsVattuHuloi
                  ?.slice()
                  ...
                  .map((row, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align="right">{row.soluongloi}</TableCell>
                        <TableCell align="right">{row.congdung}</TableCell>
                        <TableCell align="right">{row.ngaybaoloi}</TableCell>
                      </TableRow>
                    );
                  })}
                {emptyRows > 0 && (
...
export default TableVattuHuloiDanhsach;
