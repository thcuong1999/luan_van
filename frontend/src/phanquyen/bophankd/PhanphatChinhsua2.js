...
import TablePhanphatChinhsua2 from "./tables/TablePhanphatChinhsua2";
import SnackbarMaterial from "../../components/SnackbarMaterial";

const PhanphatChinhsua2 = (props) => {
  ...
  const handleRemoveRow = (id) => {
    setDsVattu(dsVattu.filter((item) => item._id !== id));
  };
  ...
  const handleChinhPhanphat = async () => {
    if (!emptyFields()) {
      ...
      // console.log({ payload });
      const data = await apiPhanphat.suaPhanphatVattu(phanphatId, {
        phanphatId,
        payload,
      });
      // console.log(data);
...
export default PhanphatChinhsua2;
