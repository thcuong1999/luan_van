...
import TablePhanphatDi2 from "./tables/TablePhanphatDi2";
import SnackbarMaterial from "../../components/SnackbarMaterial";

const PhanphatThem2 = (props) => {
  ...
  const handleThemPhanphat = async () => {
    if (!emptyFields()) {
      ...
      // console.log({ payload });
      const data = await apiPhanphat.themPhanphatVattu({ payload });
      console.log(data);
      if (data.success) {
        setAlert(true);
        setErrMsg("");
    ...
    <>
      <Wrapper>
        <Header
          ...
          titleBack
          onClick={() => props.history.push("/bophankd/vattuphanphat")}
        />
        <Content>
          <FormWrapper>
            <Form>
              <FormTitle>Phân phát vật tư</FormTitle>
              <FormGroup>
