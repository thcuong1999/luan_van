...
const HodanChinhsua = (props) => {
  ...
  const handleSubmit = async () => {
    if (!emptyFields()) {
      if (passwordMatch(hodan.matkhau, hodan.xnmatkhau)) {
        ...
        if (data.success) {
          Toastify({
            text: "Cập nhật hộ dân thành công",
            backgroundColor: "#0DB473",
            className: "toastifyInfo",
            position: "center",
          }).showToast();
          props.history.push("/giamsatvung/hodan");
        }
      }
    }
  };
  ...
  const fetchDataFirstRender = async () => {
    ...
    let diachi = hodan.diachi.split(",").map((item) => item.trim());
    setXa(diachi[0]);
    sethuyen(diachi[1]);
    setTinh(diachi[2]);
    setSelectedLangnghe(hodan.langnghe.ten);
